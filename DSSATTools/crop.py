...
default .SPE, .ECO, and .CUL files. The ecotype and cultivar parameters are defined 
as attributes of the `Crop` instance. Each parameter is accesible and can be modified
using the key, value syntax, e.g. `crop.cultivar["PARAMETER"] = VALUE`. 
...
file. If an uknown cultivar is passed, then the last cultivar in the .CUL file is
...
the `Crop.cultivar` and `Crop.ecotype` attributes respectivelly. In these two 
...
name to a value; it can be treated as a python dicitonary. Each of the different
...
Please note that only the cultivar and ecotype parameter can be modified. If the
user wants to modify the species parameters, they'll have to do it directly on the
...
class Crop:
    def __init__(self, crop_name:str='Maize', cultivar_code:str=None):
        ...
        cultivar: str
            ...
            use the DSSATTools.crop.available_cultivars function. If a new cultivar (not 
